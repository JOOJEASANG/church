...
function dayNo(){ const d=new Date(); return Math.floor(new Date(d.getFullYear(),d.getMonth(),d.getDate()).getTime()/86400000); }
function hashText(v){ const s=String(v||''); let h=2166136261; for(let i=0;i<s.length;i++){ h^=s.charCodeAt(i); h=Math.imul(h,16777619); } return h>>>0; }
function pick(){ const a=items.filter(v=>v&&v.text&&v.active!==false).sort((x,y)=>hashText(`${x.id||''}|${x.ref||''}|${x.text||''}`)-hashText(`${y.id||''}|${y.ref||''}|${y.text||''}`)||String(x.id||'').localeCompare(String(y.id||''))); return a.length?a[dayNo()%a.length]:null; }
function toast(msg){ if(typeof window.toast==='function') return window.toast(msg); alert(msg); }
function current(){ if(window.__namsanTodayVerse&&window.__namsanTodayVerse.date===today()) return window.__namsanTodayVerse; const v=pick(); if(!v) return null; const p={id:v.id||'', ref:v.ref||'오늘의 말씀', text:v.text||'', date:today(), source:'managedDailyVerse'}; window.__namsanTodayVerse=p; return p; }
...
document.addEventListener('namsan:todayVerseChanged',e=>{ if(e.detail) window.__namsanTodayVerse=e.detail; syncExisting(); });
